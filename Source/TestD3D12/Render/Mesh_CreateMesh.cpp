...
void Mesh::CreateDefaultBox() {
    ...
    Surface *surf = AllocSurface(24, 36);
    surf->materialIndex = 0;
    surfaces.Append(surf);

    constexpr float extent = BE1::MeterToUnit(0.1f);

    BE1::VertexGenericLit *v = surf->subMesh->verts;
    // bottom face
    ...
    BE1::VertIndex *idx = surf->subMesh->indexes;
    ...
    FinishSurfaces(FinishFlag::ComputeAABB | FinishFlag::ComputeTangents | Mesh::FinishFlag::ComputeEdges | FinishFlag::UseUnsmoothedTangents);
}

void Mesh::CreatePlane(const BE1::Vec3 &origin, const BE1::Mat3 &axis, float size, int numSegments) {
    ...
    Surface *surf = AllocSurface(numVerts, numIndexes);
    surf->materialIndex = 0;
    surfaces.Append(surf);
    ...
    FinishSurfaces(FinishFlag::ComputeAABB | FinishFlag::ComputeEdges);

    if (!axis.IsIdentity() || !origin.IsZero()) {
        TransformVerts(axis, BE1::Vec3::one, origin);
    }
}

void Mesh::CreateBox(const BE1::Vec3 &origin, const BE1::Mat3 &axis, const BE1::Vec3 &extents) {
    Purge();
    ...
    FinishSurfaces(FinishFlag::ComputeAABB | FinishFlag::ComputeTangents | FinishFlag::ComputeEdges | FinishFlag::UseUnsmoothedTangents);
...
void Mesh::CreateSphere(const BE1::Vec3 &origin, const BE1::Mat3 &axis, float radius, int numSegments) {
    CreateCapsule(origin, axis, radius, 0, numSegments);
}

void Mesh::CreateCylinder(const BE1::Vec3 &origin, const BE1::Mat3 &axis, float radius, float height, int numSegments) {
    ...
    BE1::Vec3 n[2] = { BE1::Vec3(0, 0, 1), BE1::Vec3(0, 0, -1) };
    ...
    for (int i = 0; i < 2; i++) {
        ...
        for (int j = 0; j < numSegments; j++) {
            float theta = BE1::Math::TwoPi * j / numSegments; // 0 ~ 2pi
            float st, ct;
            BE1::Math::SinCos(theta, st, ct);
            float x = ct * radius;
            float y = st * radius;
            float s = st * 0.5f + 0.5f;
            float t = ct * 0.5f + 0.5f;

            v->SetPosition(n[i] * half_height + BE1::Vec3(x, y, 0));
            v->SetNormal(n[i]);
            v->SetTexCoord(s, t);
            v->SetColor(0xffffffff);
            v++;
    ...
    for (int i = 0; i < numSideSegments; i++) {
        float theta = BE1::Math::TwoPi * i / (numSideSegments - 1); // 0 ~ 2pi
        float st, ct;
        BE1::Math::SinCos(theta + BE1::Math::HalfPi, st, ct);
        float x = ct * radius;
        float y = st * radius;
        float s = theta * BE1::Math::InvPi;
        BE1::Vec3 n = BE1::Vec3(ct, st, 0);
    ...
    FinishSurfaces(FinishFlag::ComputeAABB | FinishFlag::ComputeEdges | FinishFlag::ComputeTangents);
...
void Mesh::CreateCapsule(const BE1::Vec3 &origin, const BE1::Mat3 &axis, float radius, float height, int numSegments) {
    assert(numSegments >= 4);
    ...
    for (int i = 0; i < numLat; i++) {
        float phi = BE1::Math::HalfPi * i / (numLat - 1); // 0 ~ pi/2
        float sp, cp;
        BE1::Math::SinCos(phi, sp, cp);
        float z = cp * radius;
        float zr = sp * radius;
        float t = 1.0f - (z + radius + height) / real_height;

        for (int j = 0; j < numLng; j++) {
            float theta = BE1::Math::TwoPi * j / (numLng - 1); // 0 ~ 2pi
            float st, ct;
            BE1::Math::SinCos(theta - BE1::Math::HalfPi, st, ct);
            float x = ct * zr;
            float y = st * zr;
    ...
    for (int i = 0; i < numLat; i++) {
        float phi = BE1::Math::HalfPi + BE1::Math::HalfPi * i / (numLat - 1); // pi/2 ~ pi
        float sp, cp;
        BE1::Math::SinCos(phi, sp, cp);
        float z = cp * radius;
        float zr = sp * radius;
