...
public:
    ...
    Font *                  AddRefCount() { refCount++; return this; }
    const Font*             AddRefCount() const { refCount++; return this; }
    ...
    FontGlyph *             GetGlyph(char32_t unicodeChar, RenderMode renderMode = RenderMode::Normal) const;
    ...
    float                   TextWidth(const BE1::Str &text, int maxLen, bool allowLineBreak = false, bool allowColoredText = false, float xScale = 1.0f) const;
...
private:
    struct FontHashKey {
        FontHashKey() {}
        FontHashKey(const BE1::Str &name, uint32_t fontSize) : name(name), fontSize(fontSize) {}
    ...
    struct FontHashCompare {
        ...
        static bool Compare(const Type1 &lhs, const Type2 &rhs) {
            return (lhs.fontSize == rhs.fontSize && BE1::Str::Icmp(lhs.name, rhs.name) == 0);
        }
    };

    struct FontHashGenerator {
        template <typename Type>
        static int Hash(const BE1::HashIndex &hasher, const Type &value) {
            return hasher.GenerateHash(value.name, false) * value.fontSize;
        }
    };

    using FontHashMap       = BE1::HashMap<FontHashKey, Font *, FontHashCompare, FontHashGenerator>;
