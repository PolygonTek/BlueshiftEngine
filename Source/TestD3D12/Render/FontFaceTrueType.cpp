...
#define GLYPH_CACHE_TEXTURE_FORMAT  BE1::Image::Format::A8
...
struct GlyphAtlas {
    ...
    BE1::Array<stbrp_node>          nodes;
};

static BE1::Array<GlyphAtlas>       atlasArray;

static void Atlas_Add(int textureSize) {
    GlyphAtlas &atlas = atlasArray.Alloc();

    BE1::Image image;
    image.Create2D(textureSize, textureSize, 1, GLYPH_CACHE_TEXTURE_FORMAT, BE1::Image::GammaSpace::Linear, nullptr, BE1::Image::Flag::None);
    memset(image.GetPixels(), 0, image.SizeInBytes());

    atlas.texture = textureManager.AllocTexture(BE1::va("_glyph_cache_%i", atlasArray.Count() - 1));
    atlas.texture->Create(RHI::TextureType::Texture2D, &image, Texture::Flag::HighQuality | Texture::Flag::NoMipmaps);
...
bool TrueTypeFontFace::Load(const char *filename, int fontSize) {
    ...
    glyphBuffer = (byte *)Mem_Alloc16(BE1::Image::BytesPerPixel(GLYPH_CACHE_TEXTURE_FORMAT) * fontSize * fontSize * 4);

    // Pad with zeros.
    BE1::simdProcessor->Memset(glyphBuffer, 0, fontSize * fontSize * BE1::Image::BytesPerPixel(GLYPH_CACHE_TEXTURE_FORMAT));

    return true;
}

Texture *TrueTypeFontFace::RenderGlyphToAtlasTexture(char32_t unicodeChar, Font::RenderMode renderMode, int glyphPadding, int &bitmapLeft, int &bitmapTop, int &glyphX, int &glyphY, int &glyphWidth, int &glyphHeight) const {
...
FontGlyph *TrueTypeFontFace::CacheGlyph(char32_t unicodeChar, Font::RenderMode renderMode, int atlasPadding) const {
    ...
    gl->texture     = texture->AddRefCount();
...
FontGlyph *TrueTypeFontFace::GetGlyph(char32_t unicodeChar, Font::RenderMode renderMode) const {
...
void TrueTypeFontFace::WriteBitmapFiles(const char *fontFilename) {
    BE1::Str bitmapBasename = fontFilename;
    bitmapBasename.StripFileExtension();

    for (int i = 0; i < atlasArray.Count(); i++) {
        const Texture *texture = atlasArray[i].texture;

        BE1::Image bitmapImage;
        bitmapImage.Create2D(texture->GetWidth(), texture->GetHeight(), 1, texture->GetFormat(), BE1::Image::GammaSpace::DontCare, nullptr, BE1::Image::Flag::None);

        texture->GetTexels2D(0, texture->GetFormat(), bitmapImage.GetPixels(0));

        BE1::Str filename = bitmapBasename;
        filename += i;
        filename.Append(".png");
