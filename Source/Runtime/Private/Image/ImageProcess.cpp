...
// mip_gammaTable[i] = (i/255)^2.2
static float mip_gammaTable[256] = {
...
template <>
void BuildMipMap1D(byte *dst, const byte *src, const int width, const int components) {
    int xOff = (width < 2) ? 0 : components;

    for (int x = 0; x < width; x += 2) {
        for (int i = 0; i < components; i++) {
            *dst++ = Math::Ftob(255.0f * Math::Pow(0.5f * (mip_gammaTable[src[0]] + mip_gammaTable[src[xOff]]), 1.0f / 2.2f));
            src++;
        }
        src += xOff;
    }
...
template <>
void BuildMipMap2D(byte *dst, const byte *src, const int width, const int height, const int components) {
    int xOff = (width < 2) ? 0 : components;
    int yOff = (height < 2) ? 0 : components * width;
    ...
    for (int y = 0; y < height; y += 2) {
        for (int x = 0; x < width; x += 2) {
            for (int i = 0; i < components; i++) {
                *dst++ = Math::Ftob(255.0f * Math::Pow(0.25f * (mip_gammaTable[src[0]] + mip_gammaTable[src[xOff]] + mip_gammaTable[src[yOff]] + mip_gammaTable[src[yOff + xOff]]), 1.0f / 2.2f));
                src++;
            }
            src += xOff;
        }
...
template <>
void BuildMipMap3D(byte *dst, const byte *src, const int width, const int height, const int depth, const int components) {
    int xOff = (width < 2) ? 0 : components;
    int yOff = (height < 2) ? 0 : components * width;
    int zOff = (depth < 2) ? 0 : components * width * height;
    ...
    for (int z = 0; z < depth; z += 2) {
        for (int y = 0; y < height; y += 2) {
            for (int x = 0; x < width; x += 2) {
                for (int i = 0; i < components; i++) {
                    *dst++ = Math::Ftob(255.0f * Math::Pow(0.125f * (mip_gammaTable[src[0]] + mip_gammaTable[src[xOff]] + mip_gammaTable[src[yOff]] + mip_gammaTable[src[yOff + xOff]] + 
                        mip_gammaTable[src[zOff]] + mip_gammaTable[src[zOff + xOff]] + mip_gammaTable[src[zOff + yOff]] + mip_gammaTable[src[zOff + yOff + xOff]]), 1.0f / 2.2f));
                    src++;
                }
                src += xOff;
            }
