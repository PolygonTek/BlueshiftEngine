...
bool Cylinder::IntersectRay(const Ray &ray, float *hitDist) const {
    Vec3 d = p[1] - p[0];
    Vec3 m = ray.origin - p[0];
    float md = m.Dot(d);
    float nd = ray.dir.Dot(d);
    float dd = d.Dot(d);
    float scale;
    ...
    float nn = ray.dir.Dot(ray.dir);
    float mn = m.Dot(ray.dir);
    float a = dd * nn - nd * nd;
    float k = m.Dot(m) - radius * radius;
    float c = dd * k - md * md;
    // Test if ray runs parallel to cylinder axis
    if (Math::Fabs(a) < 0.000001f) {
        if (c > 0) {
