...
void OpenGLRHI::InitMainContext(WindowHandle windowHandle, const Settings *settings) {
	...
	mainContext->defaultFramebuffer = oldFBO;
...
RHI::Handle OpenGLRHI::CreateContext(RHI::WindowHandle windowHandle, bool useSharedContext) {
	...
	ctx->rootView = (EGLUtil *)windowHandle;
...
//    [ctx->eaglView setGLContext:ctx];
 ...
 //   
	...
	ctx->defaultFramebuffer = mainContext->defaultFramebuffer; // = ctx->eaglView.framebuffer;
    if (useSharedContext) {
...
//#endif
	...
	if (ctx->state != mainContext->state) {
	       // Delete default VAO for shared context
	       gglDeleteVertexArrays(1, &ctx->defaultVAO);
...
void OpenGLRHI::GetDisplayMetrics(Handle ctxHandle, int *windowWidth, int *windowHeight, int *backingWidth, int *backingHeight) {
	...
	if (windowWidth || windowHeight) {
		...
		*windowWidth = ctx->rootView->m_windowWidth;
		*windowHeight = ctx->rootView->m_windowHeight;
	}
	...
	if (backingWidth || backingHeight) {
		...
		*backingWidth = ctx->rootView->getWidth();
		*backingHeight = ctx->rootView->getHeight();
	}
}
