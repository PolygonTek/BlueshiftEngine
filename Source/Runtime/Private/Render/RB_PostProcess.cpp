    ...
    if (r_SSAO.GetBool() || r_DOF.GetBool()) {
        ...
        if (r_SSAO.GetBool()) {
            ...
            if (r_SSAO_blur.GetBool()) {
                ...
                PP_AoBlur(backEnd.ctx->ppRTs[PP_RT_AO]->ColorTexture(), backEnd.ctx->screenRT->DepthStencilTexture(), backEnd.ctx->ppRTs[PP_RT_AO_TEMP], backEnd.camera->def->projMatrix, backEnd.ctx->ppRTs[PP_RT_AO]);
            }
...
void RB_PostProcess() {
    ...
    if (r_DOF.GetBool()) {
        PP_LinearizeDepth(bc->screenRT->DepthStencilTexture(), backEnd.camera->def->zNear, backEnd.camera->def->zFar, bc->ppRTs[PP_RT_LINEAR_DEPTH]);
        linearizeDepth = true;
    }

    if (r_sunShafts.GetBool() && backEnd.primaryLight) {
        const RenderLight *sunLight = backEnd.primaryLight->def;
        if (sunLight->state.axis[0].Dot(backEnd.camera->def->state.axis[0]) < 0.0f) {
            if (!linearizeDepth) {
                linearizeDepth = true;
                PP_LinearizeDepth(bc->screenRT->DepthStencilTexture(), backEnd.camera->def->zNear, backEnd.camera->def->zFar, bc->ppRTs[PP_RT_LINEAR_DEPTH]);
            }
            ...
            Mat4 viewProjectionMatrix = backEnd.camera->def->projMatrix * backEnd.camera->def->viewMatrix.ToMat3().ToMat4();
    ...
    if (r_DOF.GetBool()) {
        ...
        PP_CopyCocToAlpha(bc->ppRTs[PP_RT_DEPTH_4X]->ColorTexture(), backEnd.camera->def->zFar, bc->ppRTs[PP_RT_4X]);

        PP_CopyColorAndCoc(bc->screenRT->ColorTexture(), bc->ppRTs[PP_RT_LINEAR_DEPTH]->ColorTexture(), backEnd.camera->def->zFar, bc->ppRTs[PP_RT_TEMP]);
    ...
    if (r_motionBlur.GetInteger() & 1) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            float c = backEnd.camera->def->viewMatrix[i].ToVec3().Dot(prevAxis[i]);
            if (c < minCos) {
                minCos = c;
            }
        }
        ...
        if (minCos < thresholdCos) {
            ...
            backEnd.viewMatrixPrev[0][3] = -(backEnd.camera->def->state.origin.Dot(prevAxis[0]));
            backEnd.viewMatrixPrev[1][3] = -(backEnd.camera->def->state.origin.Dot(prevAxis[1]));
            backEnd.viewMatrixPrev[2][3] = -(backEnd.camera->def->state.origin.Dot(prevAxis[2]));

            Mat4 viewProjMatrix[2];
            viewProjMatrix[0] = backEnd.camera->def->viewProjMatrix;
            viewProjMatrix[1] = backEnd.camera->def->projMatrix * backEnd.viewMatrixPrev;

            // 카메라 rotation 정도에 따라 iterative blur
            if (minCos > Math::Cos(DEG2RAD(4.0f))) {
                PP_PassThruPass(bc->screenRT->ColorTexture(), 0.0f, 0.0f, 1.0f, 1.0f, bc->ppRTs[PP_RT_TEMP]);
                PP_CameraMotionBlur(bc->ppRTs[PP_RT_TEMP]->ColorTexture(), bc->screenDepthTexture, viewProjMatrix, backEnd.camera->def->state.origin, 1.0f, bc->frameTime, bc->screenRT);
            } else if (minCos > Math::Cos(DEG2RAD(8.0f))) {
                PP_CameraMotionBlur(bc->screenRT->ColorTexture(), bc->screenDepthTexture, viewProjMatrix, backEnd.camera->def->state.origin, 1.0f, bc->frameTime, bc->ppRTs[PP_RT_TEMP]);
                PP_CameraMotionBlur(bc->ppRTs[PP_RT_TEMP]->ColorTexture(), bc->screenDepthTexture, viewProjMatrix, backEnd.camera->def->state.origin, 0.5f, bc->frameTime, bc->screenRT);
            } else {
                PP_PassThruPass(bc->screenRT->ColorTexture(), 0.0f, 0.0f, 1.0f, 1.0f, bc->ppRTs[PP_RT_TEMP]);
                PP_CameraMotionBlur(bc->ppRTs[PP_RT_TEMP]->ColorTexture(), bc->screenDepthTexture, viewProjMatrix, backEnd.camera->def->state.origin, 1.0f, bc->frameTime, bc->screenRT);
                PP_CameraMotionBlur(bc->screenRT->ColorTexture(), bc->screenDepthTexture, viewProjMatrix, backEnd.camera->def->state.origin, 0.5f, bc->frameTime, bc->ppRTs[PP_RT_TEMP]);
                PP_CameraMotionBlur(bc->ppRTs[PP_RT_TEMP]->ColorTexture(), bc->screenDepthTexture, viewProjMatrix, backEnd.camera->def->state.origin, 0.25f, bc->frameTime, bc->screenRT);
            }
        }
    }
