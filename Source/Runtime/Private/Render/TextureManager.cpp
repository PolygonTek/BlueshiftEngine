...
void TextureManager::Cmd_DumpTexture(const CmdArgs &args) {
    ...
    Image bitmapImage;
    Image::GammaSpace gammaSpace = texture->GetFlags() & Texture::Flag::SRGBColorSpace ? Image::GammaSpace::sRGB : Image::GammaSpace::Linear;
    bitmapImage.Create(texture->GetWidth(), texture->GetHeight(), texture->GetDepth(), texture->NumSlices(), 1, texture->GetFormat(), gammaSpace, nullptr, Image::Flag::None);
