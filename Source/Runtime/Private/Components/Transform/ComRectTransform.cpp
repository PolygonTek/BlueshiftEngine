...
void ComRectTransform::Purge(bool chainPurge) {
    if (chainPurge) {
        ComTransform::Purge(chainPurge);
    }
}
...
void ComRectTransform::GetWorldCorners(Vec3 (&worldCorners)[4]) const {
    ...
    ALIGN_AS32 Mat3x4 worldMatrix = GetWorldMatrix();

    for (int i = 0; i < COUNT_OF(localCorners); i++) {
        worldCorners[i] = worldMatrix.TransformPos(localCorners[i]);
    }
}
