...
#define bx_mm_neg_ps(a)                 _mm_xor_ps(a, _mm_castsi128_ps(_mm_set1_epi32(0x80000000))) // _mm_sub_ps(_mm_setzero_ps(), (a))

// Inverts 128 bits.
#define bx_mm_inv_si128(a)              _mm_xor_si128(a, _mm_set1_epi32(0xFFFFFFFF))

// Splats 4x32 bits floats using 2 bits selector.
#define bx_mm_splat_ps(a, i)            _mm_castsi128_ps(_mm_shuffle_epi32(_mm_castps_si128(a), _MM_SHUFFLE(i, i, i, i)))

// Permutates 4x32 bits floats using 4x2 bits selector.
#define bx_mm_perm_ps(a, imm)           _mm_castsi128_ps(_mm_shuffle_epi32(_mm_castps_si128(a), imm))

// Shifts load for two 128 bits.
// dst = (a >> imm*8) | (b << (16 - imm)*8)
#define bx_mm_sld_ps(a, b, imm)         _mm_castsi128_ps(_mm_or_si128(_mm_srli_si128(_mm_castps_si128(a), imm), _mm_slli_si128(_mm_castps_si128(b), 16 - (imm))))

// Shifts load for two 128 bits.
// dst = (a >> imm*8) | (b << (16 - imm)*8)
#define bx_mm_sld_si128(a, b, imm)      _mm_or_si128(_mm_srli_si128(a, imm), _mm_slli_si128(b, 16 - (imm))))

#ifdef __FMA__
    // dst = a * b + c
    #define bx_mm_madd_ps(a, b, c)      _mm_fmadd_ps(a, b, c)
    // dst = a * b - c
    #define bx_mm_msub_ps(a, b, c)      _mm_fmsub_ps(a, b, c)
    // dst = -(a * b) + c
    #define bx_mm_nmadd_ps(a, b, c)     _mm_fnmadd_ps(a, b, c)
    // dst = -(a * b) - c
    #define bx_mm_nmsub_ps(a, b, c)     _mm_fnmsub_ps(a, b, c)
#else
    // dst = a * b + c
    #define bx_mm_madd_ps(a, b, c)      _mm_add_ps(_mm_mul_ps(a, b), c)
    // dst = a * b - c
    #define bx_mm_msub_ps(a, b, c)      _mm_sub_ps(_mm_mul_ps(a, b), c)
    // dst = -(a * b) + c
    #define bx_mm_nmadd_ps(a, b, c)     _mm_sub_ps(c, _mm_mul_ps(a, b))
    // dst = -(a * b) - c
    #define bx_mm_nmsub_ps(a, b, c)     _mm_sub_ps(bx_mm_neg_ps(_mm_mul_ps(a, b)), c)
#endif
