...
BE_FORCE_INLINE size_t select_min_256ps(const avxf &a) { return CountTrailingZeros((uint32_t)_mm256_movemask_ps(a == vreduce_min_256ps(a))); }

// Returns index of maximum component.
BE_FORCE_INLINE size_t select_max_256ps(const avxf &a) { return CountTrailingZeros((uint32_t)_mm256_movemask_ps(a == vreduce_max_256ps(a))); }
...
BE_FORCE_INLINE size_t select_min_256ps(const avxf &a, const avxb &validmask) {
    ...
    return CountTrailingZeros((uint32_t)_mm256_movemask_ps(_mm256_and_ps(validmask, (v == vreduce_min_256ps(v)))));
}
...
BE_FORCE_INLINE size_t select_max_256ps(const avxf &a, const avxb &validmask) {
    const avxf v = select_256ps(set1_256ps(-FLT_INFINITY), a, validmask);
    return CountTrailingZeros((uint32_t)_mm256_movemask_ps(_mm256_and_ps(validmask, (v == vreduce_max_256ps(v)))));
}
