...
const SignalDef Serializable::SIG_PropertyChanged("Serializable::PropertyChanged", "ssi");
const SignalDef Serializable::SIG_PropertyArrayCountChanged("Serializable::PropertyArrayCountChanged", "ss");
const SignalDef Serializable::SIG_PropertyUpdated("Serializable::PropertyUpdated");

Serializable::Serializable(Object *owner) {
    this->owner = owner;
}
...
Variant Serializable::GetPropertyDefault(const char *name) const {
    ...
    if (!owner->GetPropertyInfo(name, propertyInfo)) {
...
Variant Serializable::GetProperty(const char *name) const {
    ...
    if (propertyInfo.accessor) {
        propertyInfo.accessor->Get(owner, out);
        return out;
    }

    const void *src = reinterpret_cast<const byte *>(owner) + propertyInfo.offset;
...
Variant Serializable::GetProperty(const char *name, int index) const {
    ...
    if (propertyInfo.accessor) {
        propertyInfo.accessor->Get(owner, index, out);
        return out;
    }
...
bool Serializable::SetPropertyWithoutSignal(const char *name, const Variant &var) {
    ...
    if (propertyInfo.accessor) {
        ...
        void *dest = reinterpret_cast<byte *>(owner) + propertyInfo.offset;
...
bool Serializable::SetProperty(const char *name, const Variant &var) {
    if (SetPropertyWithoutSignal(name, var)) {
        owner->EmitSignal(&Serializable::SIG_PropertyChanged, owner->ClassName(), name, -1);
        return true;
    }
    return false;
}
...
bool Serializable::SetPropertyWithoutSignal(const char *name, int index, const Variant &var) {
    ...
    if (propertyInfo.accessor) {
        propertyInfo.accessor->Set(owner, index, newVar);
    } else {
        void *dest = reinterpret_cast<byte *>(owner) + propertyInfo.offset;
...
bool Serializable::SetProperty(const char *name, int index, const Variant &var) {
    if (SetPropertyWithoutSignal(name, index, var)) {
        owner->EmitSignal(&Serializable::SIG_PropertyChanged, owner->ClassName(), name, index);
        return true;
    }
...
int Serializable::GetPropertyArrayCount(const char *name) const {
    ...
    if (propertyInfo.accessor) {
        return propertyInfo.accessor->GetCount(owner);
    }
...
void Serializable::SetPropertyArrayCount(const char *name, int count) {
    ...
    if (propertyInfo.accessor) {
        oldCount = propertyInfo.accessor->GetCount(owner);
        if (oldCount == count) {
            return;
        }

        propertyInfo.accessor->SetCount(owner, count);
    } else {
        void *src = reinterpret_cast<byte *>(owner) + propertyInfo.offset;
    ...
    owner->EmitSignal(&SIG_PropertyArrayCountChanged, owner->ClassName(), name);
}
