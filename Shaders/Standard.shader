shader "Lit/Standard" {
    ...
    properties {
        ...
        metallicMap("Metallic Map") : texture "2D" = "_whiteTexture" (condition _METALLIC 1 2 3 4) [linear]
        ...
        roughnessMap("Roughness Map") : texture "2D" = "_whiteTexture" (condition _ROUGHNESS 1 2) [linear]
        ...
        anisotropyMap("Anisotropy Map") : texture "2D" = "_flatNormalTexture" (condition _ANISO 2) [normal]
        ...
        clearCoatMap("Clear Coat Map") : texture "2D" = "_whiteTexture" (condition _CLEARCOAT 2) [linear]
        clearCoatScale("Clear Coat Scale") : float range 0 1.0 0.01 = "1.0" (condition _CLEARCOAT 1 2 3 4 5 6)
        _CC_ROUGHNESS("Clear Coat Roughness") : enum "Scale;Texture (R);From Metallic Map (R);From Metallic Map (G);From Metallic Map (B);From Metallic Map (A);" = "0" (condition _CLEARCOAT 1 2 3 4 5 6) [shaderDefine]
        clearCoatRoughnessMap("Clear Coat Roughness Map") : texture "2D" = "_whiteTexture" (condition _CLEARCOAT 1 2 3 4 5 6 _CC_ROUGHNESS 1) [linear]
        clearCoatRoughnessScale("Clear Coat Roughness Scale") : float range 0 1.0 0.01 = "0.0" (condition _CLEARCOAT 1 2 3 4 5 6)
        _CC_NORMAL("Clear Coat Normal") : enum "Surface Normal;Texture" = "0" (condition _CLEARCOAT 1 2 3 4 5 6) [shaderDefine]
        clearCoatNormalMap("Clear Coat Normal Map") : texture "2D" = "_flatNormalTexture" (condition _CLEARCOAT 1 2 3 4 5 6 _CC_NORMAL 1) [normal]
